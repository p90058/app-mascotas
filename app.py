...
<style>
    ...
    .btn-gps { width: 100%; padding: 1.5rem; font-size: 1.4rem; background: linear-gradient(135deg, #4CAF50 0%, #66BB6A 100%); color: white; border: none; border-radius: 12px; cursor: pointer; font-weight: bold; box-shadow: 0 4px 15px rgba(76,175,80,0.4); }
    .btn-gps:hover { opacity: 0.9; transform: scale(1.02); }
...
if 'latitud' not in st.session_state:
    st.session_state.latitud = 0.0
if 'longitud' not in st.session_state:
    st.session_state.longitud = 0.0

# CAPTURAR GPS DE URL
if "lat" in st.query_params and "lon" in st.query_params:
    try:
        st.session_state.latitud = float(st.query_params["lat"])
        st.session_state.longitud = float(st.query_params["lon"])
        st.session_state.gps_obtenido = True
        st.query_params.clear()
        st.rerun()
    except Exception as e:
        st.error(f"Error GPS: {e}")
...
with tab1:
    ...
    # GPS - IMPLEMENTACIÓN SIMPLE Y FUNCIONAL
    st.markdown('<div class="location-box">', unsafe_allow_html=True)
    st.markdown("### 📍 Ubicación GPS de la Mascota")
    
    if st.session_state.gps_obtenido:
        st.markdown(f"""
        <div class="success-box">
            ✅ UBICACIÓN CAPTURADA CORRECTAMENTE<br><br>
            📍 Latitud: {st.session_state.latitud:.6f}<br>
            📍 Longitud: {st.session_state.longitud:.6f}<br><br>
            <small>Ubicación lista para publicar</small>
        </div>
        """, unsafe_allow_html=True)
        
        if st.button("🔄 Capturar Nueva Ubicación", key="btn_nueva_ubic"):
            st.session_state.gps_obtenido = False
            st.session_state.latitud = 0.0
            st.session_state.longitud = 0.0
            st.rerun()
    else:
        # HTML/JavaScript para GPS
        st.markdown("""
        <div style="text-align: center;">
            <button id="btnGPS" class="btn-gps" onclick="obtenerGPS()">
                📍 Enviar ubicación de la mascota
            </button>
            <div id="gpsStatus" style="margin-top: 1.5rem; font-weight: bold; font-size: 1.1rem;"></div>
        </div>
        ...
        function obtenerGPS() {
            ...
            statusDiv.innerHTML = '<span style="color: #2196F3;">⏳ Solicitando permiso de ubicación...</span>';
            btn.disabled = true;
            ...
            navigator.geolocation.getCurrentPosition(
                function(position) {
                    ...
                    statusDiv.innerHTML = '<span style="color: #4CAF50; font-size: 1.3rem;">✅ ¡Ubicación obtenida!<br>Redirigiendo...</span>';
                ...
                function(error) {
                    ...
                    switch(error.code) {
                        ...
                        case error.POSITION_UNAVAILABLE:
                            ...
                            break;
                        case error.TIMEOUT:
                            mensaje = '❌ La solicitud expiró. Intenta nuevamente.';
                            break;
                        default:
        ...
        <div style="margin-top: 1.5rem; padding: 1rem; background: white; border-radius: 10px; text-align: left;">
            <b>💡 Instrucciones:</b><br>
            1. Presiona el botón verde de arriba<br>
            2. Cuando el navegador pregunte, haz clic en <b>"Permitir"</b><br>
            3. Espera a que aparezca el mensaje de éxito<br>
            4. La página se recargará automáticamente con tu ubicación
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        ...
        nombre_mascota = st.text_input("Nombre de la mascota", placeholder="Ej: Max, Luna, etc.", key="inp_nombre_mascota")
    with col2:
        color = st.text_input("Color principal", placeholder="Ej: Marrón, Negro, Blanco...", key="inp_color")
    ...
    foto = st.file_uploader("📷 Subir Foto de la mascota", type=["jpg", "png", "jpeg"], key="inp_foto")
    descripcion = st.text_area("📝 Descripción / Señas particulares", placeholder="Collar, cicatrices, comportamiento, última vez visto...", key="inp_descripcion", height=100)
    
    # BOTÓN PUBLICAR
    if st.button("🚨 PUBLICAR ALERTA", type="primary", use_container_width=True, key="btn_publicar"):
        # Validaciones
        if not nombre_usuario.strip() or not email_usuario.strip() or not telefono_usuario.strip():
            st.error("❌ Por favor completa: Nombre, Email y Teléfono")
        elif not st.session_state.gps_obtenido or st.session_state.latitud == 0.0:
            st.error("❌ Primero obtén la ubicación GPS presionando el botón verde de arriba")
        elif foto is None:
            st.error("❌ Sube una foto de la mascota")
        elif not nombre_mascota.strip():
            ...
            if not subset.empty:
                ...
                for _, row in subset.iterrows():
                    ...
                    <div class="reporte-card">
                        ...
                        <p><b>📅 Fecha de reporte:</b> {row['fecha']}</p>
...
if st.session_state.is_admin:
    with tab3:
        ...
        with admin_tab1:
            st.markdown("### Eliminar Reportes Incorrectos")
...
if not st.session_state.is_admin:
    if st.button("🔐 Acceso Administrador", key="btn_footer_admin"):
        ...
        st.rerun()
st.markdown("<div style='text-align: center; color: #999; padding: 2rem; margin-top: 2rem;'>© 2026 Red de Alerta de Mascotas 🐾</div>", unsafe_allow_html=True)
