...
with tab1:
    ...
    st.info("?? **Nota:** Permite el acceso a la ubicaci鏮 cuando el navegador lo solicite.")
    
    # 1. SENSOR GPS AUTOM糞ICO
    location = streamlit_geolocation()
    lat = location.get('latitude', None)
    lon = location.get('longitude', None)
    
    if lat and lon:
        st.success(f"? Ubicaci鏮 detectada: Lat {lat:.5f}, Lon {lon:.5f}")
    else:
        st.warning("?? Esperando permiso de ubicaci鏮... (Toca 'Allow' en tu celular)")
    ...
    with col2:
        contacto = st.text_input("Tel嶨ono de contacto")
        foto = st.file_uploader("?? Subir Foto", type=["jpg", "png", "jpeg"])

    descripcion = st.text_area("Descripci鏮 (Raza, color, se鎙s, collar)")

    if st.button("?? Publicar Alerta", type="primary"):
        if not lat or not lon:
            st.error("? No se pudo obtener la ubicaci鏮. Asegate de dar permiso al navegador.")
            ...
            with st.spinner("?? Subiendo foto y guardando datos..."):
                try:
                    ...
                    st.success("? ，lerta publicada con 憖ito!")
                    st.balloons()
...
with tab2:
    ...
    if datos:
        ...
        st.map(df[["latitud", "longitud"]]) # Mapa r嫚ido de Streamlit
        ...
        for _, row in df.iterrows():
            with st.container(border=True):
                ...
                with c2:
                    ...
                    st.markdown(f"**Descripci鏮:** {row['descripcion']}")
